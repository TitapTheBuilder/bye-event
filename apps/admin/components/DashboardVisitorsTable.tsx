  ...
  return (
    <div className="flex flex-col gap-4">
      <input
        ...
        placeholder="Search visitors to see who scanned them…"
                    ...
                    <tr key={`${visitor.id}-expanded`} className="border-b border-border-subtle bg-surface-0/40">
                      <td colSpan={4} className="px-4 py-3">
                        {loadingExpand === visitor.id ? (
                          <p className="text-sm text-text-secondary">Loading scans…</p>
                        ) : (exhibitorsById[visitor.id]?.length ?? 0) === 0 ? (
                          <p className="text-sm text-text-secondary">
                            No exhibitor has scanned this visitor yet.
                          </p>
                        ) : (
                          <ul className="flex flex-col gap-1.5">
                            {exhibitorsById[visitor.id]?.map((row) => (
                              ...
                              >
                                ...
                                <span>
                                  {row.scanCount} scan{row.scanCount === 1 ? "" : "s"} · last{" "}
