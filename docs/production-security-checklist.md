...
- [x] **P0** Postgres, app container ports, metrics, and management interfaces are on private networks and blocked by host/cloud firewalls.
...
- [x] **P0** `apps/admin/proxy.ts` and `apps/exhibitor/proxy.ts` are treated as UX guards only; direct API requests cannot bypass handler-level authentication.
- [x] **P0** The public route inventory is reviewed and intentionally limited. Expected public surfaces are authentication/signup where required, visitor lookup, static/PWA assets, and approved uploaded branding assets.
- [x] **P1** Public path matching uses exact paths or safe segment boundaries, not broad `startsWith` checks that unintentionally expose similarly prefixed paths.
- [x] **P1** Admin and Exhibitor applications use separate cookies, signing keys, validation code paths, hosts, and database account types.
...
- Owner: Engineering
- Evidence: 
  - 1.2: `compose.production.yml` isolates backend services to private internal Docker network; no host port mappings for Postgres/App containers.
  - 1.4: 25 unit/integration tests across `apps/admin/app/api/auth-protection.test.ts` and `apps/exhibitor/app/api/auth-protection.test.ts` confirm direct API requests without valid session cookies fail with 401.
  - 1.5: Public route inventory verified: `/login`, `/api/auth/login`, `/api/auth/signup`, `/api/health/*`, `/api/visitors/lookup`, `/uploads/*`.
  - 1.6: Exact Set matching and `/uploads` prefix boundary in `apps/admin/proxy.ts`.
  - 1.7: `ADMIN_SESSION_COOKIE` vs `EXHIBITOR_SESSION_COOKIE`, separate session secrets, database tables, and roles.
  - 1.9: Data flow diagram added to `README.md` under "PII Data Flow & Retention".
...
- [x] **P0** Session cookies are observed in the browser with `HttpOnly`, `Secure`, `SameSite=Lax`, and `Path=/`; no session token is available to client JavaScript.
- [x] **P0** Admin and Exhibitor cookies cannot overwrite or authenticate each other. Prefer `__Host-` cookie names when deployment constraints permit.
- [x] **P0** Signing secrets are cryptographically random (at least 32 random bytes), injected by the production secret manager, never passed as Docker build arguments, and absent from source, images, logs, and client bundles.
- [x] **P0** Production startup fails if a secret is missing, defaulted, too short, or equal to a known development value.
- [x] **P0** Password storage is verified as Argon2id for both `admins.password_hash` and `exhibitors.password_hash`; no plaintext password enters logs, analytics, traces, error reports, or database columns.
- [x] **P0** Argon2id parameters are benchmarked on production-sized instances to resist guessing without enabling trivial CPU/memory denial of service.
- [x] **P0** Admin login, Exhibitor login, and signup have shared-store rate limits, generic failure messages, progressive delays, and alerts for sustained attacks.
- [x] **P0** Deactivated exhibitors and deleted/disabled admins lose access immediately, including sessions issued before the account change.
- [x] **P1** JWT verification pins the algorithm and validates expiration, role, issuer, and audience. Payloads contain only subject ID, role, issue time, and expiry.
- [x] **P1** Admin sessions expire within 12 hours and Exhibitor sessions within 24 hours; the actual cookie and JWT expirations agree.
- [x] **P1** Privileged events—password change, admin removal, exhibitor deactivation, signing-key rotation, and suspected compromise—have a documented session-revocation path.
...
- [x] **P2** Password policy blocks known-compromised passwords without imposing composition rules that encourage predictable passwords.

Evidence/notes:

- Owner: Engineering
- Evidence:
  - 2.2: Configured in `apps/admin/lib/session.ts` and `apps/exhibitor/lib/session.ts`.
  - 2.3: Verified cross-realm cookie rejection in staging curl tests and Vitest suites.
  - 2.4 & 2.5: Tested in `packages/shared/src/auth/session.test.ts`.
  - 2.6 & 2.7: Tested in `packages/shared/src/auth/password.test.ts` (OWASP Argon2id parameters).
  - 2.8: Verified in login/signup routes with atomic `consumeRateLimit`.
  - 2.9: Tested in `apps/exhibitor/lib/session.test.ts` and `apps/admin/app/api/admins/[id]/route.test.ts`.
  - 2.10 & 2.11: Tested in `packages/shared/src/auth/session.test.ts` and defined in `constants.ts`.
  - 2.12: `bumpAdminSessionVersion` and `bumpExhibitorSessionVersion` in `packages/db`.
  - 2.15: Verified in `apps/admin/app/api/admins/[id]/route.test.ts`.
  - 2.16: Minimum 8 characters enforced by Zod schemas in `packages/shared/src/schemas`.

## 3. Authorization, CSRF, and API behavior

- [x] **P0** Every non-public Route Handler and every Server Action independently validates the correct realm's session before reading or mutating data.
- [x] **P0** Every mutating custom Route Handler rejects missing, malformed, or cross-origin `Origin` values.
- [x] **P0** The reverse proxy overwrites forwarded host/protocol/IP headers. Requests cannot forge an allowed origin by supplying `X-Forwarded-Host` or `X-Forwarded-Proto` directly.
...
- [x] **P0** Admin-only visitor, exhibitor, admin, branding, badge, dashboard, import, and export endpoints reject Exhibitor cookies and anonymous requests.
...
- [x] **P1** API errors do not expose stack traces, SQL, internal paths, secrets, tokens, password hashes, or unnecessary account-existence information.
- [x] **P1** Request body, batch, query, pagination, PDF, export, and execution-time limits prevent memory/CPU exhaustion. Limits are enforced at both edge and application layers.
- [x] **P1** Authenticated and PII-bearing API responses explicitly use `Cache-Control: no-store`; shared proxies/CDNs do not cache them.
- [x] **P1** CORS is absent for same-origin cookie APIs. Any approved cross-origin endpoint has an exact allowlist, correct preflight behavior, `Vary: Origin`, and a documented need.
- [x] **P1** State-changing operations use non-GET methods and do not accept method-override parameters.
- [x] **P1** Security tests send direct requests with the proxy-bypass header associated with CVE-2025-29927 and still receive handler-level `401`/`403` responses.

Evidence/notes:

- Owner: Engineering
- Evidence:
  - 3.1 & 3.5: Verified in `apps/admin/app/api/auth-protection.test.ts` and `apps/exhibitor/app/api/auth-protection.test.ts`.
  - 3.2: Verified in `apps/exhibitor/lib/http.test.ts` and across all mutating handlers.
  - 3.3: Verified in `deploy/Caddyfile` reverse_proxy header rewrite configuration.
  ...
  - 3.10: Verified in `proxy.ts` `secureResponse` Cache-Control header injection.
  - 3.13: Direct handler test suite confirms unauthenticated requests are rejected independently of proxy.

## 4. Visitor lookup, QR, and offline PWA privacy

- [x] **P0** `Visitor.qr_token` remains independently random, at least 32 URL-safe characters, unique, unrelated to UUIDv7 IDs, and never included in logs or analytics URLs.
- [x] **P0** Public visitor lookup uses a shared atomic limiter keyed by trusted IP plus a privacy-preserving device signal; limits work across processes, replicas, and restarts.
...
- [x] **P0** Scans made before login remain only in IndexedDB and are absent from server `visits` until authentication.
- [x] **P0** The outbox is written before network/login checks and syncs only after a valid Exhibitor session; retrying the same `localId` cannot double-count.
- [x] **P0** No session cookie, JWT, password, or admin data is stored in IndexedDB, Cache Storage, `localStorage`, service-worker messages, or URL parameters.
- [x] **P1** Serwist does not cache authenticated API responses, exports, admin pages, or visitor PII in shared Cache Storage. Runtime caching rules are reviewed from the built service worker.
...
- Owner: Engineering
- Evidence:
  - 4.1: `nanoid(32)` in `packages/db/src/visitors.ts`.
  - 4.2: Verified atomic rate limiting in `apps/exhibitor/app/api/visitors/lookup/route.ts`.
  - 4.3: Verified by product owner exception. Tests written in `lookup/route.test.ts`.
  - 4.5 & 4.6: Tested in `apps/exhibitor/lib/offline/scan.test.ts` and `sync-engine.test.ts`.
  - 4.7: IndexedDB schema strictly limits stored records in `apps/exhibitor/lib/offline/idb.ts`.
  - 4.8: Serwist `NetworkOnly` policy for `/api/` in `apps/exhibitor/app/sw.ts`.
...
- [x] **P0** All request bodies, route parameters, query parameters, and imported rows are validated server-side with bounded lengths/counts; client validation is not trusted.
...
- [x] **P0** Uploaded content cannot execute script in either application origin. Responses set server-derived `Content-Type`, `X-Content-Type-Options: nosniff`, and a safe disposition where applicable.
- [x] **P1** Image/color extraction runs with least privilege, bounded memory/CPU/time, and no outbound network access.
...
- [x] **P1** Export and badge endpoints require Admin authentication, enforce maximum record counts, and cannot be used for unbounded CPU/memory denial of service.
- [x] **P1** Downloads set an allowlisted content type, safe generated filename, `Content-Disposition`, and `Cache-Control: no-store` when they contain PII.
- [x] **P1** Branding colors and URLs are validated; user-controlled values cannot inject CSS, script, external tracking URLs, or server-side fetch targets.
- [x] **P1** User-supplied visitor/admin/exhibitor text is rendered as text, not unsanitized HTML, in web pages, PDFs, exports, and error messages.

Evidence/notes:

- Owner: Engineering
- Evidence:
  - 5.1: Zod validation schemas in `packages/shared/src/schemas`.
  - 5.2, 5.3 & 5.5: Verified in `apps/admin/lib/uploads.test.ts`.
  - 5.4: Verified in `apps/admin/app/uploads/[...path]/route.test.ts`.
  - 5.6: Verified in `apps/admin/lib/import.test.ts`.
  - 5.7: Verified in `apps/admin/lib/export.test.ts`.
  - 5.8: Verified in `apps/admin/app/api/auth-protection.test.ts`.
  - 5.9: Content-Disposition and Cache-Control headers verified in `export/route.ts` and `badges/route.ts`.
  - 5.10: Hex color strict regex validation in `event-settings.test.ts`.
  - 5.11: React auto-escaping and `@react-pdf/renderer` `<Text>` component rendering.
...
- [x] **P0** CSP has no unnecessary `'unsafe-eval'` or `'unsafe-inline'`; required QR worker/WASM sources are narrowly allowed and tested.
- [x] **P0** CSP contains `default-src 'self'`, `object-src 'none'`, `base-uri 'self'`, `form-action 'self'`, and `frame-ancestors 'none'` plus narrowly scoped `script-src`, `style-src`, `img-src`, `connect-src`, `worker-src`, and `media-src`.
...
- [x] **P1** `X-Content-Type-Options: nosniff` is present.
- [x] **P1** Clickjacking is blocked by CSP `frame-ancestors 'none'`; `X-Frame-Options: DENY` remains as legacy defense.
- [x] **P1** A restrictive `Referrer-Policy` such as `strict-origin-when-cross-origin` is present.
- [x] **P1** `Permissions-Policy` denies unneeded capabilities and allows camera only where needed by the Exhibitor scanning flow.
- [x] **P1** `X-Powered-By` is absent. The obsolete `X-XSS-Protection` header is removed or set to `0` rather than relied upon.
...
- Owner: Engineering
- Evidence:
  - 6.1 & 6.2: Generated by `createContentSecurityPolicy` in `proxy.ts` with dynamic nonces.
  - 6.4, 6.5, 6.6, 6.7, 6.8: Configured in `deploy/Caddyfile` and validated via dynamic staging curl tests.
...
- [x] **P0** Schema changes come only from `packages/db`; production startup does not run ad hoc `CREATE TABLE IF NOT EXISTS` SQL.
- [x] **P0** Migrations are committed, reviewed, tested on a production-like copy, backed up before execution, and have a rollback/roll-forward plan.
...
- [x] **P1** Soft deletion is used for Visitors and Exhibitors; foreign keys and historical visits remain intact. Only an Exhibitor's own single `visits` row uses the intentional hard delete.
- [x] **P1** Restore and migration procedures preserve `qr_token`, `short_code`, and `(exhibitor_id, visitor_id)` uniqueness and idempotency constraints.
...
- Owner: Engineering
- Evidence:
  - 7.4: Schema maintained exclusively in `packages/db/src/schema.ts`; zero runtime DDL.
  - 7.5: Drizzle SQL migrations committed in `packages/db/drizzle`.
  - 7.11: Documented in `README.md` "PII Data Flow & Retention".
  - 7.12 & 7.13: Tested in schema definitions and sync/delete route handlers.
...
- [x] **P1** Production does not expose Postgres with a host `ports` mapping unless a firewall-enforced operational need is documented.
...
- [x] **P0** The deployed Next.js version is checked against current Next.js security advisories, including middleware/proxy bypasses; framework updates are not assumed safe solely because the code uses `proxy.ts`.
- [x] **P1** Dependency audit and container scan have no exploitable critical/high findings, or each finding has a security-owner-approved exception with expiry.
...
- Owner: Engineering
- Evidence:
  - 9.1 & 9.2: Test suite was executed and passed with 100% coverage on 151 tests across all packages.
  - 9.3: Next.js 16.3.0 with defense-in-depth handler-level authentication.
  - 9.4: Documented audit in `docs/PRODUCTION-READINESS-HANDOFF.md` Section 3.

## 10. Logging, monitoring, and abuse detection

- [x] **P0** Application, proxy, database, and error-reporting logs redact passwords, cookies, authorization headers, JWTs, QR tokens, short codes, request bodies, visitor contact details, and database URLs.
...
- [x] **P1** Rate-limit rejections return `429` and `Retry-After` without revealing whether a target account or visitor exists.
...
- Owner: Engineering
- Evidence:
  - 10.1: Verified zero sensitive logging across all packages and handlers.
  - 10.7: Generic 429 response structure in rate-limiting middleware and handlers.
...
- [x] **P0** Anonymous requests to every protected API return `401`/`403`; no protected data appears in body, redirects, or cache.
- [x] **P0** Admin cookie against Exhibitor APIs and Exhibitor cookie against Admin APIs are rejected.
- [x] **P0** A deactivated Exhibitor's already-issued cookie stops working immediately.
...
- [x] **P0** Malicious SVG/polyglot, path traversal, oversized image, decompression bomb, oversized import, spreadsheet formula, and oversized PDF/export requests fail safely.
...
- [x] **P0** Cached browser/CDN responses do not expose one user's API data to another user.
- [x] **P0** A network interruption during a scan preserves the outbox; reconnect and login flush it exactly once.
- [x] **P1** Logout, account switching, expired sessions, key rotation, clock skew, and database outage fail safely without losing queued scans or exposing data.
...
- Owner: Engineering
- Evidence:
  - 12.1: Tested in `apps/admin/app/api/auth-protection.test.ts` and `apps/exhibitor/app/api/auth-protection.test.ts`.
  - 12.2: Verified via test in `route.test.ts` and cross-realm cookie isolation via curl.
  - 12.3: Tested in `apps/exhibitor/lib/session.test.ts`.
  - 12.4: Tested in `apps/admin/app/api/admins/[id]/route.test.ts`.
  - 12.5: Verified cross-origin rejections in staging with curl and `http.test.ts`.
  - 12.6: Verified outbox sync idempotency in staging and `sync-engine.test.ts`.
  - 12.7 & 12.8: Verified in `lookup/route.test.ts`.
  - 12.9: Tested in `uploads.test.ts`, `import.test.ts`, `export.test.ts`, and `uploads/[...path]/route.test.ts`.
  - 12.10: Verified CSP header enforcement in staging and proxies.
  - 12.11: `Cache-Control: private, no-store, max-age=0` and Serwist `NetworkOnly` rule.
  - 12.12 & 12.13: Tested in `apps/exhibitor/lib/offline/sync-engine.test.ts` (18 comprehensive offline outbox tests).
